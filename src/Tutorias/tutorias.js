...
import TablaTutorias from './TablaTutorias'
...
class Tutorias extends React.Component{
    constructor(props){
        ...
        this.state = {tutorias: listaTutorias};
        this.abrirAgregarTutoria = this.abrirAgregarTutoria.bind(this);
        this.abrirRegistrarTutoria = this.abrirRegistrarTutoria.bind(this);
    }

    componentDidMount() {
      console.log("componentDidMount");
      axios.get('https://localhost:44323/api/tutoriahorario')
      .then(Response =>{
        console.log('response')
        this.setState({tutorias : Response.data})
      })
      .catch(error =>{
        console.log('error')
        console.log(error)
  ...
  abrirAgregarTutoria() {
    this.props.history.push('/AgregarTutoria');
  }
  abrirRegistrarTutoria(id_tutoria) {
    //this.props.history.push('/RegistrarTutoria');

      let indice = this.state.tutorias.findIndex(tutorias => tutorias.id_tutoria === id_tutoria );
      //Carga otra página con ruta /EditarLibro
      this.props.history.push('/RegistrarTutoria',this.state.tutorias[indice]);
  }
    render() {
        return (
          <div>
    ...
    <div class="carousel-inner">
      <div class="carousel-item active">
        <img src="img/calendar-1990453.jpg" class="d-block w-100" alt="..." height="500px"/>
      </div>
    </div>
...
<div className="container-sm">
    <TablaTutorias tutorias={this.state.tutorias} abrirRegistrarTutoria={this.abrirRegistrarTutoria} />
</div>
<br/>
        <h3 >Agrega tu Horario</h3>
        <hr/>
       
       <div className="text-center">
           <button className="btn btn-primary"><i class="bi bi-file-earmark-plus-fill" onClick={this.abrirAgregarTutoria}>Registrate aquí</i></button>
           </div>   
        <hr/>
